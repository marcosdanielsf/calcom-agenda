...
import { resolveNexusBinding, usesNexusBooking } from "@calcom/lib/nexus-booking/server";
...
export const reserveSlotHandler = async ({ ctx, input }: ReserveSlotOptions) => {
  if (usesNexusBooking()) {
    resolveNexusBinding({ eventTypeId: input.eventTypeId });
    // The definitive availability check and reservation belong to Nexus, not a local hold.
    return { uid: "" };
  }
