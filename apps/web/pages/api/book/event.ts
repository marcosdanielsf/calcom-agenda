...
import { submitNexusBooking, usesNexusBooking } from "@calcom/lib/nexus-booking/server";
import { HttpError } from "@calcom/lib/http-error";

async function handler(req: NextApiRequest & { userId?: number; traceContext: TraceContext }) {
  if (req.method !== "POST") throw new HttpError({ statusCode: 405, message: "Método não permitido" });
  ...
  if (usesNexusBooking()) return submitNexusBooking(req.body, req.headers["idempotency-key"]);
