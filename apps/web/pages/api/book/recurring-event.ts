...
import { usesNexusBooking } from "@calcom/lib/nexus-booking/server";
...
async function handler(req: NextApiRequest & RequestMeta) {
  if (usesNexusBooking()) throw new HttpError({ statusCode: 400, message: "Reservas recorrentes ainda não estão disponíveis nesta agenda." });
