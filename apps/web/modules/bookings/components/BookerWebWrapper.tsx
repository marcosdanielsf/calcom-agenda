...
const BookerWebWrapperComponent = (props: BookerWebWrapperAtomProps): JSX.Element => {
  ...
  if (bookings.agendaStatus) {
    return (
      <section role="status" aria-live="polite" className="bg-default border-subtle mx-auto my-16 max-w-lg rounded-lg border p-8 text-center">
        <h1 className="text-emphasis text-2xl font-semibold">{t(`agenda_reservation_${bookings.agendaStatus}_title`)}</h1>
        <p className="text-default mt-4">{t(`agenda_reservation_${bookings.agendaStatus}_description`)}</p>
        {bookings.agendaRecoveryError && <p role="alert" className="text-default mt-4">{bookings.agendaRecoveryError}</p>}
        {bookings.agendaStatus === "processing" && (
          <div className="mt-6 flex flex-wrap justify-center gap-2">
            <Button loading={bookings.checkingAgendaStatus} onClick={bookings.refreshAgendaStatus}>{t("agenda_reservation_check")}</Button>
            <Button color="secondary" disabled={bookings.checkingAgendaStatus} onClick={bookings.returnToBookingForm}>{t("agenda_reservation_resume")}</Button>
          </div>
        )}
        {bookings.agendaStatus === "rejected" && <Button className="mt-6" onClick={bookings.returnToBookingForm}>{t("agenda_reservation_choose_slot")}</Button>}
      </section>
    );
  }
